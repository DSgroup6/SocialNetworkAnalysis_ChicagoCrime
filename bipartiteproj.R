...
new_network <- proj_i + igraph::vertices(isolates)# add isolated nodes to the network
...
network_new <- snafun::add_vertex_attributes(new_network, value = df_crimenet)

#check consistency
snafun::extract_all_vertex_attributes(network_new)[2,]
node_info[8,]

snafun::g_density(network_new)
### store the network 
##save('network_new', file="crime_net.rda")


## try ERGM

#dgwnsp(decay= 0.1, fixed=FALSE )
...
crime_ERGM <- ergm::ergm(crime_net ~ edges + nodecov("perc_unemp") + 
                           ...
                           nodecov("med_age") + nodecov("perc_black") + gwdsp(decay= 0.1, fixed=FALSE )+
                           degree(34) + degree(37), 
                         control = ergm::control.ergm(MCMC.burnin = 3000,
...
snafun::v_stress(network_i)
snafun::v_betweenness(network_i)
