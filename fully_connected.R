...
network_i <- snafun::add_edge_attributes(network_i, c('weight'), weights.exp) # 1/weight because the closer, the stronger the connection
...
# control
model1.lm <- lm(THEFT ~ perc_unemp + med_age + LT_HS + HS + SOME_COLL + ASSOC + BACH + GRAD_PROF + perc_black + perc_asian + perc_white + perc_hispanic 
                + perc_inc_under_25 + perc_inc_25_50 +  perc_inc_25_50 +  perc_inc_50_75 + perc_inc_75_100 + perc_inc_100_150, data = attrs)
model1.lm2 <- lm(THEFT ~ perc_unemp + med_age + LT_HS + HS + ASSOC + BACH + GRAD_PROF + perc_black + perc_asian + perc_white + perc_hispanic 
                + perc_inc_under_25 + perc_inc_25_50 +  perc_inc_25_50 +  perc_inc_50_75 + perc_inc_75_100 + perc_inc_100_150, data = attrs)
texreg::screenreg(list(model1,model1.lm), custom.model.names = c('lnam', 'lm'))
